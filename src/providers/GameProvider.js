...
// import { db } from '../services/puguessr-firebase.js';
import { database } from '../services/puguessr-firebase.js';
...
export function setIsUserInLobby(lobbyId, user, value) {
  let lobbyRef = database.ref(`/lobbies/${lobbyId}/players`);
  if (lobbyRef) {
    lobbyRef.update({
      [user.uid]: {
        inLobby: value,
...
export function addGuessedLoc(uid, username, lobbyID, round, lat, lon, points) {
  database.ref(`games/${lobbyID}/rounds/${round}/results/${uid}`).set({
    username: username,
    lat: lat,
    lon: lon,
    points: points,
...
export function addFinalResult(uid, username, lobbyID, points) {
  database.ref(`games/${lobbyID}/total/${uid}`).set({
    username: username,
    points: points,
  });
}
...
export function deleteGame(lobbyID) {
  database.ref(`games/${lobbyID}`).set({
    deleted: null,
  });
}
...
export function createGame(
  ...
  database.ref(`games/${lobbyID}`).update({
    host: uid_host,
    timeLimit: timeLimit,
    maxRounds: rounds,
    lobby: lobbyID,
  ...
  //add partecipants
  users.forEach((user) => {
    database.ref(`games/${lobbyID}/partecipants`).update({
      [user.uid]: user.username,
    });
  });
  //create rounds
  database.ref(`games/${lobbyID}/rounds`).set({
    null: null,
  });
  for (let index = 0; index < rounds; index++) {
    ...
    database.ref(`games/${lobbyID}/rounds`).update({
      [index + 1]: roundData,
    });
  }
  //set start so people get redirected
  database.ref(`lobbies/${lobbyID}`).update({
    gameStarted: true,
    gameID: lobbyID,
  });
}
